...
프로세스 실행 중 인터럽트가 발생하거나, 실행 중인 CPU 사용 허가시간을 모두 소모하거나, 입출력을 위해 대기해야 하는 경우 
다른 프로세스가 CPU를 점유하게 되면 기존에 동작중인 프로세스의 정보(PCB)를 저장하고 다시 CPU를 점유했을 때, 이전에 실행되던 프로세스 상태를 복구해야 함
