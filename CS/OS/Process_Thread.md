...
프로세스가 자신에게 할당된 CPU 시간을 전부 사용하거나 인터럽트 발생으로 인해 다른 프로세스가 CPU를 점유하게 되면
기존에 동작중인 프로세스의 상태를 보관하고 다시 CPU를 점유했을 때, 이전에 보관했던 프로세스 상태를 복구해야 함
특징 : Cache 초기화. Memory mapping 초기화, 커널 항시 실행
...
### 장점
- 독립적인 프로세스에 비해 공유 메모리만큼의 시간, 자원 손실 감소 및 전역, 정적 변수에 대한 자료 공유 가능
### 단점
...
## [3. 동기화]
### 동기화란?
> 일반적으로 프로세스와 스레드는 비동기적으로 수행됨 (처리 순서가 보장되지 않음)<br/>
> 여러 프로세스 혹은 스레드가 동시에 동작할 때 발생되는 아래와 같은 문제점을 해결하기 위한 기법
